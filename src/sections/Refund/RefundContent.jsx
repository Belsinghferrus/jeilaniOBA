...
const RefundContent = () => {
  const { t } = useLanguage();
  const sections = t.refundPage.sections;
            ...
            <div className="md:sticky md:top-32">
              <div className="flex items-center space-x-3 mb-6">
                ...
                <span className="text-[#D4AF37] font-bold tracking-[0.2em] uppercase text-xs">
                  {t.refundPage.toc.title}
                </span>
              </div>

              <nav className="border-l border-gray-200 pl-6">
                ...
                <article key={section.id} id={section.id} className="scroll-mt-32">
                  ...
                  <div className="space-y-5">
                    {section.blocks.map((block, bIndex) => {
                      if (block.type === 'p') {
                        return (
                          <p key={bIndex} className="text-gray-700 leading-relaxed text-base">
                            {block.text}
                          </p>
                        );
                      }
                        ...
                        return (
                          <ul key={bIndex} className="space-y-3 pl-2">
                            {block.items.map((item, iIndex) => (
                              <li key={iIndex} className="flex items-start gap-3 text-gray-700 leading-relaxed text-base">
                      ...
                      if (block.type === 'contact') {
                        return (
                          <div key={bIndex} className="bg-offwhite border-l-2 border-[#D4AF37] p-6 md:p-8 space-y-3">
                            {block.items.map((item, cIndex) => (
                              <div key={cIndex} className="flex flex-col md:flex-row md:items-baseline gap-1 md:gap-4">
                                ...
                                {item.value && (
                                  <span className="text-gray-700 text-base">{item.value}</span>
                                )}
                              </div>
                            ))}
                          </div>
